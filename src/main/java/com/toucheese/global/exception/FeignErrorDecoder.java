        ...
        return switch (response.status()) {
            case 400 -> {
                if (methodKey.contains("getAppleToken")) {
                    ...
                    yield new ToucheeseJwtException(ErrorCode.INVALID_APPLE_ACCESS_TOKEN);
                }
                if (methodKey.contains("revokeAppleAuthToken")) {
                    // 애플 서버에서 토큰 무효화 실패 시
                    yield new ToucheeseJwtException(ErrorCode.APPLE_REVOKE_TOKEN_FAIL);
                }
                if (methodKey.contains("getAppleTokenForRevoke")) {
                    // Authorization Code 만료 관련 오류 처리
                    yield new ToucheeseJwtException(ErrorCode.AUTHORIZATION_CODE_EXPIRED);
                }
                // 기본 400 에러 처리
                yield new ToucheeseJwtException(ErrorCode.APPLE_AUTH_TOKEN_FAIL);
            }
            case 401 -> {
                if (methodKey.contains("getAppleToken")) {
